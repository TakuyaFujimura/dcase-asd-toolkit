...
from asdkit.utils.config_class import DMConfig, DMSplitConfig
    ...
    def val_dataloader(self):
        if self.dm_cfg.valid is not None:
            ...
            if self.dm_cfg.valid.dataloader.get("shuffle", True):
                logger.warning(
                    "Validation dataloader set shuffle=True, which is not recommended"
                )
            if self.dm_cfg.valid.batch_sampler is not None:
                logger.warning(
                    "Validation batch_sampler is not None, which is not recommended"
                )
            if self.dm_cfg.valid.collator.get("shuffle", True):
                logger.warning(
                    "Validation collator set shuffle=True, which is not recommended"
                )
